...
class CSVTracker:
    """Track CSV files created during benchmark execution and generate a summary"""

    def __init__(self):
        self.created_files = []

    def register_csv(self, file_path: Path, description: str, category: str = "Data"):
        """Register a CSV file with description"""
        self.created_files.append({
            'file': file_path.name,
            'path': str(file_path.relative_to(Path.cwd()) if file_path.is_absolute() else file_path),
            'description': description,
            'category': category,
            'size_mb': round(file_path.stat().st_size / 1024 / 1024, 2) if file_path.exists() else 0
        })

    def generate_summary_markdown(self, output_file: Path = None):
        """Generate a markdown summary of all created CSV files"""
        if not self.created_files:
            return

        if output_file is None:
            output_file = RESULTS_DIR / "csv_files_summary.md"
        ...
        content = []
        content.append("# CSV Files Generated by JFR Benchmark")
        content.append("")
        content.append(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        content.append(f"Total files: {len(self.created_files)}")
        content.append("")

        # Summary table
        content.append("## Summary")
        content.append("")
        content.append("| Category | Files | Total Size (MB) |")
        content.append("|----------|-------|-----------------|")

        for category, files in categories.items():
            total_size = sum(f['size_mb'] for f in files)
            content.append(f"| {category} | {len(files)} | {total_size:.2f} |")

        content.append("")

        # Detailed tables by category
        for category, files in sorted(categories.items()):
            content.append(f"## {category} Files")
            content.append("")
            content.append("| File Name | Description | Size (MB) |")
            content.append("|-----------|-------------|-----------|")

            for file_info in sorted(files, key=lambda x: x['file']):
                content.append(f"| `{file_info['file']}` | {file_info['description']} | {file_info['size_mb']} |")
...
class BenchmarkRunner:
    def __init__(self, minimal=False, threads=None, max_retries=2, verbose=False, dynamic_queue_size=False):
        ...
        self.csv_tracker = CSVTracker()  # Initialize CSV tracker
...
"""
        ...
        try:
            ...
            if lost_stats_matches:
                ...
                if total_lost_samples > 0:
                    ...
                    # If we have lost samples data but no loss percentage yet, try to calculate it
                    # by looking for successful samples count in the log
                    if 'loss_percentage' not in result and total_lost_samples > 0:
                        print(f"    🔍 Attempting to calculate loss percentage from LOST_SAMPLE_STATS data...")

                        # Look for successful samples count in the log
                        successful_samples_match = re.search(r'(?:Successful|Total|Sample)\s*Samples?:\s*([\d,]+)', content, re.IGNORECASE)
                        if successful_samples_match:
                            successful_samples = int(successful_samples_match.group(1).replace(',', ''))
                            total_attempted = successful_samples + total_lost_samples
                            if total_attempted > 0:
                                loss_percentage = (total_lost_samples / total_attempted) * 100
                                result['loss_percentage'] = loss_percentage
                                print(f"    📊 Calculated loss percentage: {loss_percentage:.2f}% ({total_lost_samples}/{total_attempted})")
                            else:
                                print(f"    ⚠️ Total attempted samples is 0")
                        else:
                            # Fallback: assume a reasonable sampling rate to estimate total attempted
                            # This is very rough but better than no result
                            estimated_total_attempted = total_lost_samples * 10  # Assume 10% loss rate as baseline
                            loss_percentage = (total_lost_samples / estimated_total_attempted) * 100
                            result['loss_percentage'] = loss_percentage
                            print(f"    📊 Estimated loss percentage (fallback): {loss_percentage:.2f}% (assuming 10% baseline)")
            ...
            if section_match:
                ...
                # Format the stats text for better readability
                formatted_stats = stats_text.replace('\\n', '\n    ').strip()
                print(f"    📝 JFR Statistics:")
                print(f"    ┌{'─' * 50}┐")
                for line in formatted_stats.split('\n'):
                    if line.strip():
                        print(f"    │ {line.strip():<48} │")
                print(f"    └{'─' * 50}┘")
                ...
                if successful_match and total_match and lost_match:
                    ...
                    if (total + lost) > 0:
                        ...
                        print(f"       • Successful: {successful:>12,} samples")
                        print(f"       • Total:      {total:>12,} samples")
                        print(f"       • Lost:       {lost:>12,} samples")
                        print(f"       • Attempted:  {total + lost:>12,} samples")
                        print(f"       • Loss Rate:  {loss_percentage:>12.3f}%")
                        if 'loss_percentage' not in result:
                            result['loss_percentage'] = loss_percentage
    ...
    def run_native_benchmark(self):
        ...
        print(f"   • Estimated Runtime:   {time_str:>8}")
        print(f"   • Expected Completion: {datetime.fromtimestamp(time.time() + total_seconds).strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"   • Configuration:       {len(self.queue_sizes)} queues × {len(self.sampling_intervals)} intervals × {len(self.native_durations)} durations × {len(self.stack_depths)} stack depths")
                ...
                for duration in self.native_durations:
                    for stack_depth in self.stack_depths:
                        ...
                        if current_test > 1:
                            ...
                            print(f"\n┌─ Progress: {progress:5.1f}% │ Test [{current_test:>3}/{total_tests}] │ ETA: {eta_time} ─┐")

                        print(f"│   Queue: {queue_size:>4} │ Interval: {interval:>3}ms │ Duration: {duration:>3}s │ Stack: {stack_depth:>3} │")
                        ...
                        if result and result.get('success', False) and result.get('loss_percentage') is not None:
                            print(f"│ ✅ Result: {result['loss_percentage']:>6.2f}% loss rate │ Duration: {test_time:>5.1f}s                    │")
                            ...
                            print(f"│ ❌ Failed: No data collected │ Duration: {test_time:>5.1f}s                      │")
                            if result:
                                result['test_number'] = current_test
                                result['test_duration_actual'] = test_time
                                self.update_csv_realtime('native', result)
        ...
        print(f"\n┌─" + "─" * 50 + "┐")
        print(f"│ ✅ Native Benchmark Complete!                     │")
        print(f"│ • Total Runtime: {total_time/3600:>5.1f} hours ({total_time/60:>5.1f} minutes)      │")
        print(f"│ • Results saved to benchmark_results/data/        │")
        print(f"└─" + "─" * 50 + "┘")
    ...
    def run_renaissance_benchmark(self):
        ...
        print(f"   • Estimated Runtime:   {time_str:>8}")
        print(f"   • Expected Completion: {datetime.fromtimestamp(time.time() + total_seconds).strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"   • Configuration:       {len(self.queue_sizes)} queues × {len(self.sampling_intervals)} intervals")
        ...
        for queue_size in self.queue_sizes:
            for interval in self.sampling_intervals:
                ...
                if current_test > 1:
                    ...
                    eta_time = datetime.fromtimestamp(time.time() + eta_seconds).strftime('%H:%M:%S')
                    progress = (current_test - 1) / total_tests * 100
                    print(f"\n┌─ Progress: {progress:5.1f}% │ Test [{current_test:>2}/{total_tests}] │ ETA: {eta_time} ─┐")

                print(f"│ 🏛️ Queue: {queue_size:>4} │ Interval: {interval:>3}ms │ Renaissance Suite             │")
    ...
    def plot_loss_kinds(self, df: pd.DataFrame, progress_mode: bool = False):
        ...
        self.csv_tracker.register_csv(csv_path, f"Loss kinds breakdown by category ({test_type})", "Loss Analysis")
        ...
        for interval in intervals:
            ...
            if not interval_data.empty:
                ...
                self.csv_tracker.register_csv(interval_csv_path, f"Loss kinds for {interval} interval ({test_type})", "Loss Analysis")        # Create individual plots for each interval
        for interval in intervals:
            interval_data = loss_df[loss_df['interval'] == interval]
    ...
    def plot_vm_operations(self, df: pd.DataFrame, progress_mode: bool = False):
        ...
        if vm_ops_data:
            ...
            self.csv_tracker.register_csv(csv_path, f"VM operations breakdown ({test_type})", "VM Operations")            # Save interval-specific CSVs
            for interval in intervals:
                interval_data = vm_ops_df[vm_ops_df['interval'] == interval]
                if not interval_data.empty:
                    ...
                    interval_data.to_csv(interval_csv_path, index=False)
                    self.csv_tracker.register_csv(interval_csv_path, f"VM operations data ({interval})", "VM Operations")
    ...
    def plot_renaissance_out_of_thread_percentage(self, drain_df: pd.DataFrame, progress_mode: bool = False):
        ...
        if not merged_data.empty:
            ...
            self.csv_tracker.register_csv(csv_path, "Renaissance out-of-thread drainage data", "Thread Analysis")
            ...
            for interval in merged_data['interval'].unique():
                ...
                if not interval_data.empty:
                    ...
                    interval_data.to_csv(interval_csv_path, index=False)
                    self.csv_tracker.register_csv(interval_csv_path, f"Renaissance out-of-thread data ({interval})", "Thread Analysis")
        else:
    ...
    def plot_queue_size_distribution_percentiles(self, df: pd.DataFrame, test_type: str, progress_mode: bool = False):
        ...
        if not test_drain_data.empty:
            ...
            if distribution_data:
                ...
                self.csv_tracker.register_csv(csv_path, f"Queue size distribution data ({test_type})", "Queue Analysis")
        ...
        for test_type in ['native', 'renaissance']:
            ...
            if category_found:
                ...
                if csv_data:
                    ...
                    csv_df.to_csv(csv_file, index=False)
                    self.csv_tracker.register_csv(csv_file, f"Signal handler duration by queue size ({test_type})", "Signal Analysis")
                ...
                if len(queue_sizes) > 1:
                    ...
                    if not progress_mode and data_found:
                        ...
                        if memory_csv_data:
                            ...
                            self.csv_tracker.register_csv(csv_file, f"Memory consumption by queue size ({test_type})", "Memory Analysis")
                        else:
        ...
        if len(queue_sizes) > 1:
            ...
            if data_found:
                ...
                if not progress_mode:
                    ...
                    self.csv_tracker.register_csv(csv_file, f"Loss kinds by queue size ({test_type})", "Loss Analysis")
        ...
        for test_type in ['native', 'renaissance']:
            ...
            if category_found:
                ...
                if csv_data:
                    ...
                    csv_df.to_csv(csv_file, index=False)
                    self.csv_tracker.register_csv(csv_file, f"Drainage duration by queue size ({test_type})", "Queue Analysis")
    ...
    def plot_vm_ops_loss_by_queue_size(self, df: pd.DataFrame, progress_mode: bool = False):
        ...
        if any_data_found:
            ...
            if csv_data:
                ...
                csv_df.to_csv(csv_file, index=False)
                self.csv_tracker.register_csv(csv_file, f"VM operations loss by queue size ({test_type})", "VM Operations")
...
def main():
    ...
    try:
        ...
        if args.all or args.visualize:
            ...
            # Generate CSV summary markdown after visualizations
            try:
                runner.csv_tracker.generate_summary_markdown()
            except Exception as e:
                print(f"⚠️ Could not generate CSV summary: {e}")
