...
void fill_call_trace_given_top(JavaThread* thd,
  ...
  for (; count < depth && !st.at_end(); st.next(), count++) {
    ...
    if (st.is_java_frame()) {
      uint8_t type = JFRLL_FRAME_JAVA;
      if (st.is_native_frame()) {
        type = JFRLL_FRAME_NATIVE;
      } else if (st.is_inlined()) {
        type = JFRLL_FRAME_JAVA_INLINED;
      }
...
void fill_call_trace_for_non_java_thread(JFRLL_CallTrace *trace, jint depth, void* ucontext, bool include_c_frames) {
  ...
  if (!frame_from_context(&ret_frame, ucontext)) {
    if ((trace->kind & JFRLL_UNKNOWN_TRACE) == 0) {
      trace->num_frames = JFRLL_WRONG_KIND;
      return;
    }
    trace->kind = JFRLL_UNKNOWN_TRACE;
    trace->num_frames = JFRLL_UNKNOWN_NOT_JAVA; // -3
    return;
  }
...
void asyncGetStackTraceImpl(JFRLL_CallTrace *trace, jint depth, void* ucontext, int32_t options) {
  assert(trace->frames != NULL, "");
  bool include_c_frames = (options & JFRLL_INCLUDE_C_FRAMES) != 0;
  bool include_non_java_threads = (options & JFRLL_INCLUDE_NON_JAVA_THREADS) != 0;
  bool walk_during_unsafe_states = (options & JFRLL_WALK_DURING_UNSAFE_STATES) != 0;
  bool walk_same_thread = (options & JFRLL_WALK_SAME_THREAD) != 0;
  ...
  if (walk_same_thread) {
    ...
    if (tl == nullptr) {
      trace->num_frames = JFRLL_NO_THREAD;
      return;
    }
  ...
  if (raw_thread == NULL || !raw_thread->is_Java_thread()) {
    trace->kind = raw_thread == NULL ? JFRLL_UNKNOWN_TRACE : JFRLL_CPP_TRACE;
    ...
    if (include_non_java_threads) {
      ...
      trace->num_frames = (jint)JFRLL_THREAD_NOT_JAVA; // -10
    }
    return;
  }

  trace->kind = JFRLL_JAVA_TRACE; // 0
  ...
  if ((thread = JavaThread::cast(raw_thread))->is_exiting()) {
    trace->num_frames = (jint)JFRLL_THREAD_EXIT; // -8
    trace->state = JVMTI_THREAD_STATE_TERMINATED;
    return;
  }

  if (!walk_during_unsafe_states && thread->is_at_poll_safepoint()) {
    trace->num_frames = (jint)JFRLL_UNSAFE_STATE; // -12
    return;
  }

  if (thread->in_deopt_handler()) {
    trace->kind = JFRLL_DEOPT_TRACE;
    if ((trace->kind & kind_mask) == 0) {
      trace->num_frames = JFRLL_WRONG_KIND;
      return;
    }
    if (include_non_java_threads) {
      ...
      trace->num_frames = (jint)JFRLL_DEOPT; // -9
    }
    return;
  }
  ...
  if (Universe::heap()->is_gc_active()) {
    ...
    if (include_non_java_threads) {
      ...
      trace->num_frames = (jint)JFRLL_GC_ACTIVE; // -2
    }
    return;
  }

  if (!JvmtiExport::should_post_class_load()) {
    trace->num_frames = (jint)JFRLL_NO_CLASS_LOAD; // -1
    return;
  }
  ...
  if (check_state && (trace->state & state_mask) == 0) {
    trace->num_frames = JFRLL_WRONG_STATE;
    return;
  }
  ...
  case _thread_new_trans:
    ...
    if (walk_during_unsafe_states && include_c_frames) {
      trace->kind = JFRLL_NEW_THREAD_TRACE;
      if ((trace->kind & kind_mask) == 0) {
        trace->num_frames = JFRLL_WRONG_KIND;
        return;
      }
  ...
  case _thread_in_vm_trans:
    {
      ...
      if (!thread->pd_get_top_frame_for_signal_handler(&ret_frame, ucontext, false)) {
        if (!include_c_frames || !thread->pd_get_top_frame_for_profiling(&ret_frame, ucontext, false, true)) {
          trace->num_frames = (jint)JFRLL_UNKNOWN_NOT_JAVA; // -3
          return;
        }
      } else {
        if (!thread->has_last_Java_frame()) {
          if (!include_c_frames) {
            trace->num_frames = (jint)JFRLL_NO_JAVA_FRAME; // 0
            return;
          }
        } else {
          trace->num_frames = JFRLL_NOT_WALKABLE_NOT_JAVA;    // -4 non walkable frame by default
        }
      }
  ...
  case _thread_in_Java_trans:
    {
      ...
      if (!thread->pd_get_top_frame_for_profiling(&ret_frame, ucontext, true, include_c_frames)) {
        ...
        if (!include_c_frames || !thread->pd_get_top_frame_for_profiling(&ret_frame, ucontext, true, false)) {
          trace->num_frames = (jint)JFRLL_UNKNOWN_JAVA; // -5
          return;
        }
      }
      trace->num_frames = JFRLL_NOT_WALKABLE_JAVA; // -6
      fill_call_trace_given_top_with_thread(thread, trace, depth, ret_frame,
        !include_c_frames);
    }
    break;
  default:
    ...
    trace->num_frames = (jint)JFRLL_UNKNOWN_STATE; // -7
    break;
  }
}
...
public:
  AsyncGetStackTraceCallBack(JFRLL_CallTrace *trace, jint depth, void* ucontext, int32_t options) :
...
void AsyncGetStackTrace(JFRLL_CallTrace *trace, jint depth, void* ucontext, int32_t options) {
  ...
  if (walk_same_thread) {
    ...
    trace->num_frames = JFRLL_UNKNOWN_STATE;
#ifdef ASSERT
    asyncGetStackTraceImpl(trace, depth, ucontext, options);
#else
